 ...
 */
class OrcQuerySuite extends QueryTest with OrcTest with BeforeAndAfterAll {
      ...
      withSQLConf(SQLConf.ORC_VECTORIZED_READER_ENABLED.key -> value) {
        withSQLConf(SQLConf.ORC_FILTER_PUSHDOWN_ENABLED.key -> "true") {
          ...
          withTempPath { file =>
            // It needs to repartition data so that we can have several ORC files
            // in order to skip stripes in ORC.
            spark.createDataFrame(data).toDF("a").repartition(10).write.orc(file.getCanonicalPath)
  ...
  test("read from multiple orc input paths") {
    Seq("false", "true").foreach { value =>
      withSQLConf(SQLConf.ORC_VECTORIZED_READER_ENABLED.key -> value) {
        ...
        assertResult(20)(spark.read.orc(path1.getCanonicalPath, path2.getCanonicalPath).count())
      }
    }
  }
}
